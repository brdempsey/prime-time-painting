...
export const Navigation = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white shadow-md flex justify-between items-center p-4 z-50 font-bold">
      ...
      <div className="hidden md:flex items-center space-x-8">
        ...
        <div className="relative">
          <button 
            ...
            className={`text-black ${servicesOpen ? "font-bold text-red-700" : ""}`}
          >
            Services
          </button>
          {servicesOpen && (
            <div className="absolute left-0 bg-white shadow-md mt-2 space-y-2 py-2 w-48">
              <Link href="#" className="block px-4 text-black hover:bg-gray-100" onClick={() => setServicesOpen(false)}>
                Design
              </Link>
              <Link href="#" className="block px-4 text-black hover:bg-gray-100" onClick={() => setServicesOpen(false)}>
                Interior
        ...
        <div className="relative">
          <button 
            ...
            className={`text-black ${aboutOpen? "font-bold text-red-700" : ""}`}
          >
            About Us
          </button>
          {aboutOpen && (
        ...
        <Link href="#" className={pathname === "/gallery" ? "font-bold text-red-700" : "text-black"}>
          Gallery
        </Link>

        <Link href="#" className={pathname === "/contactus" ? "font-bold text-red-700" : "text-black"}>
          Contact Us
        </Link>
      ...
      >
        <Link href="/" className="text-black py-2" onClick={() => setMenuOpen(false)}>
          Home
        </Link>
        <div className="relative">
          <button 
          ...
          {servicesOpen && (
            <div className="absolute left-0 bg-white shadow-md mt-2 space-y-2 py-2 w-48 transition-all duration-300 ease-in-out transform">
              <Link href="/design" className="block px-4 text-black hover:bg-gray-100" onClick={() => setServicesOpen(false)}>
                Design
              </Link>
              <Link href="/interior" className="block px-4 text-black hover:bg-gray-100" onClick={() => setServicesOpen(false)}>
                Interior
              </Link>
              <Link href="/exterior" className="block px-4 text-black hover:bg-gray-100" onClick={() => setServicesOpen(false)}>
                Exterior
              </Link>
            </div>
          )}
        ...
        <div className="relative">
          ...
          {aboutOpen && (
            <div className="absolute left-0 bg-white shadow-md mt-2 space-y-2 py-2 w-48 transition-all duration-300 ease-in-out transform">
              <Link href="#" className="block px-4 text-black hover:bg-gray-100" onClick={() => setAboutOpen(false)}>
                Our Mission
              </Link>
              <Link href="#" className="block px-4 text-black hover:bg-gray-100" onClick={() => setAboutOpen(false)}>
                Meet Our Team
        ...
        <Link href="/gallery" className="text-black py-2" onClick={() => setMenuOpen(false)}>
          Gallery
        </Link>
        <Link href="/contactus" className="text-black py-2" onClick={() => setMenuOpen(false)}>
          Contact Us
        </Link>
        <a href="tel:+15555555555">
          <button className="bg-[#1976d2] text-white py-2 px-4 rounded hover:bg-[#1565c0] transition duration-200">
